...
// asserts on `projects` still needs the whole graph to build its fixtures.
import { readCatalogue, buildRows, assertOverridesAreLive } from '../../../../scripts/tenant-rows.mjs';

/**
 * Every `projects` tenant table, proved isolated one table at a time.
 *
 * **Why this suite exists.** `services/projects` owns 32 tenant tables and had
 * no isolation suite. Their policies ran only when a `host` route happened to
 * reach them — and measured against a fully seeded stack after a 61-screen
 * browser pass, **21 of the 32 are never WRITTEN by anything**. Their
 * `WITH CHECK` — the half that stops one tenant planting a row in another's
 * books — was exercised by nothing at all.
 *
 ...
 *
 * Ten `projects` tables deliberately withhold DELETE, and six of those withhold
 * UPDATE too — they are append-only records (a lead activity, a design review,
 * a handover). That is a STRONGER guarantee than a policy, because the
 * privilege does not exist at all, and the write tests below assert whichever
 * of the two applies rather than assuming every table is fully granted.
 */
...
async function runtimeClient(): Promise<Client> {
  ...
  const c = new Client({
    ...
    password: 'runtime_pw',
    database: 'cog',
    application_name: 'projects-isolation',
  });
  await c.connect();
  return c;
}
