...
**Package:** `@cog/procurement`  ·  **Tier:** Domain service  ·  **Owner:** `@construct-o-genie/backend`  ·  **Status:** scaffold — no implementation yet

---

## Responsibility

vendors, purchase orders, PO items, inventory items, stock transfers, vendor retention ledger, vendor performance ratings.
...
Does not pay anyone. Payment requests, remittance and TDS are `services/finance`.

## What ports in from the legacy app

`src/modules/purchase-orders/`, `src/modules/vendors/`, `POsView.js`, `app/lib/api/purchase-orders/`, `InventoryView.js`. Owns CGST/SGST (9+9 intra-state) vs IGST (18 inter-state) determination.
