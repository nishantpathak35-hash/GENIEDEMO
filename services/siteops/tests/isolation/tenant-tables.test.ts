...
// asserts on `siteops` still needs the whole graph to build its fixtures.
import { readCatalogue, buildRows, assertOverridesAreLive } from '../../../../scripts/tenant-rows.mjs';

/**
 * Every `siteops` tenant table, proved isolated one table at a time.
 *
 * **Why this suite exists.** `services/siteops` owns 5 tenant tables and had no
 * isolation suite. Their policies ran only when a `host` route happened to
 * reach them — and measured against a fully seeded stack after a 61-screen
 * browser pass, **3 of the 5 are never WRITTEN by anything**:
 * `imprest_requests`, `measurement_records` and `site_recces`. Their
 * `WITH CHECK` — the half that stops one tenant planting a measurement in
 * another's records — was exercised by nothing at all.
 *
 ...
 *
 * `measurement_records` withholds both DELETE and UPDATE — a recorded
 * measurement is an append-only fact. That is a STRONGER guarantee than a
 * policy, because the privilege does not exist at all, and the write tests
 * below assert whichever of the two applies rather than assuming every table is
 * fully granted.
 */
...
async function runtimeClient(): Promise<Client> {
  ...
  const c = new Client({
    ...
    password: 'runtime_pw',
    database: 'cog',
    application_name: 'siteops-isolation',
  });
  await c.connect();
  return c;
}
