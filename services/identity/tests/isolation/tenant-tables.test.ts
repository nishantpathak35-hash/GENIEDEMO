...
// asserts on `identity` still needs the whole graph to build its fixtures.
import { readCatalogue, buildRows, assertOverridesAreLive } from '../../../../scripts/tenant-rows.mjs';

/**
 * Every `identity` tenant table, proved isolated one table at a time.
 *
 * **Why this suite exists.** `services/identity` owns 7 tenant tables and had
 * no isolation suite, which is the worst place in the repo to have one missing:
 * these tables decide who a principal is and what they may do. Two of the seven
 * are outside the policy regime by design and are handled by `EXCLUDED` below.
 *
 ...
 *
 * Both must be read BEFORE a tenant is known — resolving a bearer key or a
 * provider identity is what establishes the context — so no tenant predicate
 * can apply to them. They are protected by PRIVILEGE instead: no grant to
 * `app_runtime` at all, reachable only through a SECURITY DEFINER function.
 *
 * A name cannot be added here to silence a failure. `the bootstrap tables are
 * protected by privilege instead` below asserts that each entry really does
 * have zero policies AND no grant to `app_runtime` — so a table with a broken
 * policy, listed here to make it go quiet, fails on the grant instead.
 */
const EXCLUDED = new Set<string>(['identity.principal_lookup', 'identity.invite_lookup']);
 ...
 *
 * Every `identity` table in scope is fully granted to `app_runtime`. The write
 * tests still consult the grant rather than assuming it, so a table that later
 * withholds DELETE is held to the stronger outcome automatically.
 */
...
async function runtimeClient(): Promise<Client> {
  ...
  const c = new Client({
    ...
    password: 'runtime_pw',
    database: 'cog',
    application_name: 'identity-isolation',
  });
  await c.connect();
  return c;
}
...
describe('the suite covers what it claims to', () => {
  ...
  it('the bootstrap tables are protected by privilege instead', async () => {
    // The self-policing half of EXCLUDED. An entry must have NO tenant policy
    // (otherwise it should have been FORCEd and tested like the rest) and NO
    // grant to `app_runtime` — privilege is the whole of its protection, so if
    // the grant ever appears the table is exposed with nothing standing in the
    // way and this test is the only thing that would say so.
    const c = await runtimeClient();
    try {
      for (const excluded of EXCLUDED) {
        const bare = excluded.slice(SCHEMA.length + 1);
        const policies = await c.query<{ n: number }>(
          `SELECT count(*)::int AS n FROM pg_policies WHERE schemaname = $1 AND tablename = $2`,
          [SCHEMA, bare],
        );
        expect({ table: excluded, policies: policies.rows[0]?.n }).toEqual({
          table: excluded,
          policies: 0,
        });
        ...
        const granted = await c.query<{ n: number }>(
          `SELECT count(*)::int AS n FROM information_schema.role_table_grants
            WHERE grantee = 'app_runtime' AND table_schema = $1 AND table_name = $2`,
          [SCHEMA, bare],
        );
        expect({ table: excluded, grants: granted.rows[0]?.n }).toEqual({
          table: excluded,
          grants: 0,
        });

        // And the privilege really does bite, not merely appear absent.
        await expect(
          asTenant(c, TENANT_A, () => c.query(`SELECT 1 FROM ${SCHEMA}.${bare} LIMIT 1`)),
